...
const TaskForm = () => {
  ...
  useEffect(() => {
    if (id) {
      const fetchTask = async () => {
        const res = await axios.get(`https://week-4-integrating-the-mern-stack.onrender.com/api/tasks/${id}`);
        setTask(res.data);
      };
      fetchTask();
    }
  ...
  const handleSubmit = async e => {
    e.preventDefault();
    if (id) {
      await axios.put(`https://week-4-integrating-the-mern-stack.onrender.com/api/tasks/${id}`, task);
    } else {
      await axios.post('https://week-4-integrating-the-mern-stack.onrender.com/api/tasks', task);
    }
    navigate('/');
  };
